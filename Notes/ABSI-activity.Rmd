...
coef(fpower)
```
For simplicity of calculation, then, let's insist that the exponent be 2 and fit the model:
```{r}
fsimple = fitModel( Weight ~ A*Height^2, data=body)
coef(fsimple)
plotPoints(Weight ~ Height, data=body)
plotFun(fpower(Height)~Height,add=TRUE,col="red")
plotFun(fsimple(Height)~Height,add=TRUE,col="black")
```
...
Although BMI may be useful for indicating risk of premature death, there are aspects of body shape that BMI does not capture.  In particular, fit people with relatively high muscle mass and low body fat have a misleadingly high BMI --- muscle is denser than body fat.

A simple way to operationalize the information contained in muscle mass and body fat is to consider waist circumference (WC) along with height and weight.  There are many ways to do this.  One approach is to modify the BMI index to include waist circumference.  But how to do this?

A simple modification is based on the idea that higher waist circumference for a given BMI increases risk of premature death: Multiply the BMI by waist circumference to create an $mBMI$ index,
$$ mBMI_0 = BMI \times WC .$$
Multiplication, rather than addition, is appropriate because of the units issue. WC has different units than BMI, so if they were being added, some conversion of WC to the units of BMI would be called for.  

Another approach is to construct an index that includes WC, but which doesn't duplicate the information that's already in BMI. Such an index has been proposed by [Krakauer and Krakauer](http://www.plosone.org/article/info%3Adoi/10.1371/journal.pone.0039504).  They call it "A Body Shape Index" (ABSI) and describe it as a way of adjusting WC for BMI and height, so that the ABSI index avoids overlap with BMI and height.  Their definition is:
$$ABSI = \frac{WC}{BMI^{2/3}height^{1/2}} .$$
In interpreting this formula, think of $BMI^{2/3}height^{1/2}$ as a model for the information in WC that is already contained in BMI and height.  By comparing the actual WC to the model WC, the ABSI index attempts to present the information in WC that is not already contained in BMI and height.
