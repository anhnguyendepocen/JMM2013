...
The parameters $A$ and $B$ describe the shape of the ellipse.  $A$ is a typical radius (the ``semi-latus rectum'', if you must know) and $B$ is the {\em eccentricity}.  When the eccentricity is zero, the ellipse is a circle.
...
# This is where it would be nice to have a plotPath() function, e.g.
# plotPoints( c(r*cos(angs),r*sin(angs))~angs, angs.lim=c(0,2*pi))
```
...
* The area of the pie-shaped space swept out, approximating it as a triangle, is $\frac{1}{2} r^2 d\theta$.
